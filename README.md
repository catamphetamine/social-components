 ...
 * `skipPostQuoteBlocks: boolean?` — Skips all "block" (not "inline") post quotes.
 * `skipGeneratedPostQuoteBlocks: boolean?` — Skip all autogenerated "block" (not "inline") post quotes. Is `true` by default.
 * `skipAttachments: boolean?` — Skips any attachments (embedded and non-embedded). Is `true` by default.
 * `skipNonEmbeddedAttachments: boolean?` — Skips non-embedded attachments. Is `true` by default.
 * `skipUntitledAttachments: boolean?` — Skips untitled attachments (embedded and non-embedded). Is `true` by default.
