 ...
 * @param  {object} post
 * @param  {(any|function)} postLinkTest — The condition for removing a post link: either a post id or a function that returns `true` when the post link should be removed.
 */
	...
	if (content) {
		...
		if (Array.isArray(paragraph)) {
			...
			if (typeof firstPart === 'object' &&
				...
				const postLinkHasQuotes = isPostLinkQuote(firstPart)
				const postLinkIsGeneratedQuote = isPostLinkGeneratedQuote(firstPart)
				...
				if (!postLinkHasQuotes || postLinkIsGeneratedQuote) {
...
function shouldRemovePostLink(postLink, postLinkTest) {
	...
	throw new Error(`Unsupported \`postLinkTest\` argument: ${postLinkTest}`)
}