 ...
 * @param  {object} [options.messages] — An object of shape `{ contentType: { audio: 'Audio', ... } }`. Currently only `messages.contentType` are used.
	...
	if (attachments && (options.skipAttachments === false) && (options.skipNonEmbeddedAttachments === false)) {
		...
		if (options.skipUntitledAttachments === false) {
			if (messages && messages.contentType) {
				for (const attachment of attachments) {
					const text = getAttachmentTypeLabel(attachment, messages.contentType)
					if (text) {
